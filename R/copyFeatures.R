...
#' }
#' @export
copyFeatures = function(file, minframes, source=c("Phase", "Trackmate")) {
  source <- match.arg(source)
  if (source == "Phase") {
  	full_ft <- read.csv(file, header=TRUE, skip=1, stringsAsFactors = FALSE)
  	full_ft$ROI_filename <- sprintf("%s-%s", full_ft$Frame, full_ft$Tracking.ID)
  	out <- full_ft[, c("Frame", "Tracking.ID", "ROI_filename", "Volume..µm..", "Sphericity...")]
  	colnames(out) <- c("FrameID", "CellID", "ROI_filename", "Volume", "Sphericity")
  } else if (source == "Trackmate") {
  	full_ft <- read.csv(file, header=TRUE, stringsAsFactors = FALSE)
