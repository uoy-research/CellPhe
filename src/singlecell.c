...
int main (int argc, char *argv[])
{
  ...
  {
    fscanf(fp, "%f %f %d %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n", &X, &Y, &xpix, &ypix, &Volume, &Thickness, &Radius, &Area, &Sphericity, &Length, &Width, &Orientation, &Mass, &Displacement, &Velocity,  &Vel1,  &Vel2, &TrackLength);
    framenum = dtmp - 1;
    if (startframe == -1) startframe = framenum;
    framenum -= startframe; 
    ...
    if (TrackLength > 0.0) input[10].frame[framenum] = Displacement/TrackLength;
...
void minBox(BOUND *boundary, INVARS *input, int inputnum, int nframes, int *missingframe)
{
  ...
  for (k = 0; k < nframes; k++)
  {
    if (missingframe[k] != 1)
    {
      ...
      for (j = 0; j < boundary[k].blength; j++)
      {
        roty = keepy1 - sin(alpha)*(float)(boundary[k].xpix[j] - keepx1) + cos(alpha)*(float)(boundary[k].ypix[j] - keepy1);
        rotx = keepx1 + cos(alpha)*(float)(boundary[k].xpix[j] - keepx1) + sin(alpha)*(float)(boundary[k].ypix[j] - keepy1);
     ...
     /*   if (k == 0) printf("%d %f %f\n",j, rotx, roty);*/
      ...
      xlength = maxx-minx;
      ylength = maxy-miny;
      input[inputnum].frame[k] = (float)(xlength*ylength)/input[13].frame[k];
      max = xlength;
...
void varFromCentre(BOUND *boundary, INVARS *input, int inputnum, int nframes, int *missingframe)
{
  ...
  for (k = 0; k < nframes; k++)
  {
    ...
    else input[inputnum].frame[k] = -1.0;
  }
  free(dist);
}
 ...
 *******************************************************************************/
 void atob(BOUND *boundary, INVARS *input, int inputnum, int nframes, int *missingframe)
 {
    ...
    for (k = 0; k < nframes; k++)
    {
      if (missingframe[k] != 1)
      {
        input[inputnum].frame[k] = input[inputnum-1].frame[k]/(float)(boundary[k].blength*boundary[k].blength);
      }
      else input[inputnum].frame[k] = -1.0;
    }
  ...
  for (k = 0; k < nframes; k++)
  {
    if (missingframe[k] != 1)
    {
      ...
      for(q = 0.1; q < 0.95; q+=0.1)
      {
        ...
        term1 = term1 + (boundary[k].xpix[boundary[k].blength - 1]*boundary[k].ypix[0]);
        term2 = term2 + (boundary[k].xpix[0]*boundary[k].ypix[boundary[k].blength - 1]);
        polyarea = 0.5 * abs(term1 + (boundary[k].xpix[0] * boundary[k].ypix[0]) - term2 - (boundary[k].xpix[0] * boundary[k].ypix[0]));
...
void summarystats(INVARS *input, int numinput, int nframes, int *missingframe)
{
  ...
  for (j = 0; j < nframes; j++)
  {
    if (missingframe[j] == 1) nummissing++;
  }
    ...
    for (k = 0; k < numinput; k++)
    {
      ...
      for (j = 0; j < nframes; j++)
      {
        if (missingframe[j] != 1)
        {
          ...
          m3 += (input[k].frame[j]-m1) * (input[k].frame[j]-m1) * (input[k].frame[j]-m1);
        }
      }
      /* stat 1 is standard deviation */
      input[k].stats[1] = sqrt(m2/enm1);
      /* stat 2 is skewness */
      if (m2 == 0.0) input[k].stats[2] = 0.0;
      else input[k].stats[2] = (m3 * en * sqrt(enm1))/(enm2 * m2 * sqrt(m2));
    }
  }
}
...
double timeSeriesVars(INVARS *input, int numinput, int nframes, int *missingframe)
{
  ...
  for (j = 0; j < numinput; j++)
  {
    ...
    for (k = 1; k < nframes; k++)
    {
      ...
      if (diff > 0) input[j].vars[0] += diff;
      else input[j].vars[1] += diff;
      if (input[j].frame[k] > input[j].vars[2]) input[j].vars[2] = input[j].frame[k];
    }
